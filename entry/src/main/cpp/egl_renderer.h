...
public:
    ...
    bool Init(OHNativeWindow* window, uint32_t toplevelId, int w, int h);
    void Shutdown();
...
private:
    bool SwitchPresenter(winehua::FramePresenterPath requestedPath, bool fallbackActive, const std::string& reason);
    void PublishPresenterState(const std::string& note = std::string()) const;
    void RenderLoop();
    ...
    std::unique_ptr<winehua::IFramePresenter> presenter_;
    winehua::BackendCaps presenterCaps_;
    bool presenterFallbackActive_ = false;
    std::string presenterSelectionNote_;
