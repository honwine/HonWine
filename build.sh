...
#
# 用法:
#   ./build.sh full          # 全量构建 (含依赖)
#   ./build.sh deps          # 只构建交叉编译依赖
...
case "${1:-all}" in
    all|full)
        bash "$ROOT/scripts/build_deps.sh"
        ...
        ;;
    deps)     bash "$ROOT/scripts/build_deps.sh" ;;
    ...
    quick)
        ...
        ;;
    *)
        echo "Usage: $0 {all|full|deps|wine|box64|assemble|hnp|hap|deploy|quick} [device_ip]"
        echo ""
        echo "  all       全量构建 (首次使用)"
        echo "  full      同 all"
        echo "  deps      只构建交叉编译依赖 → sysroot-ext"
        echo "  wine      只构建 Wine"
